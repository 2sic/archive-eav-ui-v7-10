...
angular.module('eavEditTemplates', []).run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('fields/entity/entity-default.html',
    "<div class=eav-entityselect><div ui-tree=options data-empty-placeholder-enabled=false ng-show=\"chosenEntities.length > 0\"><ol ui-tree-nodes ng-model=chosenEntities entity-validation ng-required=false ng-class=\"{'with-selector' : to.settings.merged.AllowMultiValue}\"><li ng-repeat=\"item in chosenEntities track by $index\" ui-tree-node class=eav-entityselect-item><div ui-tree-handle><i title=\"{{ 'FieldType.Entity.DragMove' | translate }}\" class=\"icon-tag pull-left eav-entityselect-sort\" ng-show=to.settings.Entity.AllowMultiValue></i> <span title=\"{{getEntityText(item) + ' (' + item + ')'}}\">{{getEntityText(item)}}</span> <span class=eav-entityselect-item-actions><span data-nodrag title=\"{{ 'FieldType.Entity.Edit' | translate }}\" ng-click=\"edit(item, index)\"><i class=icon-pencil></i></span> <span data-nodrag title=\"{{ 'FieldType.Entity.Remove' | translate }}\" ng-click=\"removeSlot(item, $index)\" class=eav-entityselect-item-remove><i ng-class=\"{ 'icon-minus-circled': to.settings.merged.AllowMultiValue, 'icon-down-dir': !to.settings.merged.AllowMultiValue  }\"></i></span></span></div></li></ol></div><select class=\"eav-entityselect-selector form-control input-lg\" formly-skip-ng-model-attrs-manipulator ng-model=selectedEntity ng-change=addEntity() ng-show=\"to.settings.merged.AllowMultiValue || chosenEntities.length < 1\" ng-class=\"{ 'with-list': chosenEntities.length != 0}\"><option value=\"\" translate=FieldType.Entity.Choose></option><option value=new ng-if=createEntityAllowed() translate=FieldType.Entity.New></option><option ng-repeat=\"item in availableEntities\" ng-disabled=\"chosenEntities.indexOf(item.Value) != -1\" value={{item.Value}}>{{item.Text}}</option></select></div>"
  );
